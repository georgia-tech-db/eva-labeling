...
class LabelStudioMLManager(object):
        ...
        **init_kwargs,
        ...
        if not issubclass(model_class, EvaLabelingBase):
            raise ValueError(
                ...
                + EvaLabelingBase.__class__.__name__
            )
    ...
    def fetch(cls, project=None, label_config=None, force_reload=False, **kwargs):
        ...
        if (
            not cls._current_model
            or (
                model_version != cls._current_model.model_version
                and model_version is not None
            )
            or os.getenv("AUTO_UPDATE", default=AUTO_UPDATE_DEFAULT)
        ):
    ...
    def _get_models_from_workdir(cls, project):
        ...
        final_models = []
        for subdir in map(
            int, filter(lambda d: d.isdigit(), os.listdir(project_model_dir))
        ):
            job_result_file = os.path.join(
                project_model_dir, str(subdir), "job_result.json"
            )
